...
function submit_form(fid, page, contid, reload, cb, xdata) {
  ...
  container.html(
    '<h6 class="text-center" > <i class="fa fa-spin fa-spinner"></i> Request Processing</h6>'
  );
 ...
 * @param {data} xdata 
 */
function submit_payment_form(fid, page, contid, reload, cb, xdata) {
  var form = document.getElementById(fid);
  var formData = new FormData(form);
  if (xdata) {
    for (var i = 0, len = xdata.length; i < len; i++) {
...
function makeRequest(url, domId, method, xData=null) {
  alert("i am here");
  $("#" + domId).html(
    '<h2 style="padding:50px;text-align:center; margin:30px; background:#fff;">Loading... <br> <img width="48"  src="/images/loading.gif" /> </h2>'
  );
