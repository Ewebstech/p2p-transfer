...
function validationError($data, $httpCode)
{
    return response()->json([
        ...
        'message' => \Modules\ParrotHttp\Http\Controllers\ErrorClassifier::responseMessage($data['responseCode'] ?? '01'),
        'data' => $data ?? []
    ], $httpCode);
}

function standardResponse($data, $httpCode, $processAfterResponse=false)
{
    $message = $data['message'] ?? '';
    $response = [
        'responseCode' => $data['responseCode'] ?? '404',
        'message' => \Modules\ParrotHttp\Http\Controllers\ErrorClassifier::responseMessage($data['responseCode'] ?? '404', $message),
        'data' => $data ?? []
    ];
    \Log::info("Response in " . getCurrentAction() . " :" . print_r($response, true));

    if($processAfterResponse == true){
        respondOK($response);
    } else {
...
function exceptionResponse($data, $httpCode)
{
    $response = [
        ...
        'message' => \Modules\ParrotHttp\Http\Controllers\ErrorClassifier::responseMessage($data['responseCode'] ?? '91', $data['message']),
        'data' => $data ?? []
    ];
    \Log::info("Exception Response in " . getCurrentAction() ." :" . print_r($response, true));

    return response()->json($response, $httpCode);
}

function getResponseMessage($responseCode, $message=''){
    return \Modules\ParrotHttp\Http\Controllers\ErrorClassifier::responseMessage($responseCode, $message);
}
...
function generate_years($startAt = null, $count = 20, $format = YEAR_GENERATOR_CENTER)
{
    ...
    switch (strtoupper($format)) {
        ...
        case YEAR_GENERATOR_TOP:
            for ($i = 0; $i < $count; $i++) {
                $years[] = $startAt - $i; json_encode($data);
                json_encode($data);
                json_encode($data);
            }
            break;
